...
Ou seja, um valor `logical` ao ser gravado com números inteiros (`integer`) será modificado para `integer` (`TRUE` para `1` e `FALSE` para `0`); um número inteiro é modificado para `numeric`; um número será modificado para `character` se for salvo com outros valores `character` (`1` para `"1"`).
...
## Usando algoritmos com expressões lógicas

Na programação é muito importante o uso de expressões lógicas para a realização de algumas tarefas. A estrutura das expressões lógicas é basicamente **SE** um fato ocorrer **ENTÃO** fazer determinada ação. No R, utilizamos o `if` para isso. Veja o exemplo abaixo. 
...
Outro aspecto importante para quem trabalha com programação é o uso de loopings (iterações). Para isso, utilizaremos a ferramenta `for()`. Essa ferramenta é útil para que seja utilizado um determinado algoritmo **POR** `n` vezes. Veja o exemplo abaixo:
...
Ou poderíamos construir uma função em que nos retornaria o valor da área de um retângulo, logo após inserirmos dois valores na função. Veja como construir tal função:
 Para isso, utilizaremos a função `function()` e devemos descrever quais **argumentos** deverão ser utilizadas pela função. No nosso caso, os argumentos são base (`b`) e altura (`h`). Para descrever como é o algoritmo, abra e feche as linhas de comando com chaves (`{}`).
...
## Funções mais complexas (mesclando iterações, expressões lógicas e outras coisas)
